...
def bim_attack(image, model, epsilon=0.01, alpha=0.005, num_iter=10):
    # 签名：bim_attack(image: Tensor, model: Module, epsilon: float, alpha: float, num_iter: int) -> Tensor
...
def main():
    ...
    for image_path in image_paths:
        ...
        # BIM 攻击
        perturbed_image = bim_attack(image_tensor, model, epsilon, alpha, num_iter)

        # 保存攻击后的图像
        file_name = os.path.splitext(os.path.basename(image_path))[0]
        new_file_path = os.path.join(output_folder, f'{file_name}_BIM.jpg')
