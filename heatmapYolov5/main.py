...
parser.add_argument('--img-path', type=str, default='imag', help='input image path')
parser.add_argument('--output-dir', type=str, default='outputs4', help='output dir')
...
def folder_main(folder_path):
    ...
    for item in os.listdir(folder_path):
        ...
        for i, mask in enumerate(masks):
            ...
            res_img, heat_map = get_res_img(bbox, mask, res_img)
            res_img = put_text_box(bbox, cls_name, res_img)
            heat_map = put_text_box(bbox, cls_name, heat_map)
            # images.append(res_img)
            # final_image = concat_images(images)
            img_name = split_extension(os.path.split(img_path)[-1], suffix='-res')
            sonoutput_dir=os.path.join(args.output_dir,img_name)
            img_name = split_extension(os.path.split(img_path)[-1], suffix='-res' + istr)
            output_path = f'{sonoutput_dir}/{img_name}'
            os.makedirs(sonoutput_dir, exist_ok=True)
            print(f'[INFO] Saving the final image at {output_path}')
            cv2.imwrite(output_path, res_img)
            mask_name = split_extension(os.path.split(img_path)[-1], suffix='-heat_map' + istr)
            maskoutput_path = f'{sonoutput_dir}/{mask_name}'
            cv2.imwrite(maskoutput_path, heat_map)
